...
const CartScreen = () => {
  ...
  const totalPrice = coffeeCart.coffee.reduce((acc, item) => acc + item.price * item.quantity, 0)


  return (
    <ScrollView className='mt-10 w-full h-full'>
      <SafeAreaView>
        ...
        />
        <View className='flex-row  justify-end mr-2'> <Text className='font-bold text-lg mt-1 text-center'>Total: {totalPrice} TL</Text> </View>
      </SafeAreaView>
    </ScrollView>
  )
}
